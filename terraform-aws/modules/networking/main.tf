...
resource "aws_subnet" "app_private" {
  ...
  tags = merge(
    var.tags,
    {
      Name = "${var.prefix}-app-private-subnet-${element(["a", "b", "c", "d", "e", "f", "g", "h", "i", "j"], count.index)}-${var.environment}"
    }
  )
}
...
# Route table para subnets privadas da aplicação
resource "aws_route_table" "app_private" {
  ...
  tags = merge(
    var.tags,
    {
      Name = "${var.prefix}-app-private-rt-${var.environment}"
    }
  )
}

resource "aws_route_table" "rds_private" {
  #count  = length(var.rds_private_subnets_pbet)
  vpc_id = aws_vpc.main.id
  ...
  tags = merge(
    var.tags,
    {
      #Name = "rds-private-rt-pbet-hml-${element(["a", "b", "c", "d", "e", "f", "g", "h", "i", "j"], count.index)}"
